...
class DurationStepMonitor:
    ...
    def append_duration_steps_to_list(self) -> None:
        if self._duration_steps != 0:
            ...
            self._update_average_duration_steps()
            self._reset_duration_steps()

    def _reset_duration_steps(self) -> None:
        self._duration_steps = 0

    def _update_average_duration_steps(self) -> None:
        sample_epochs = self._duration_steps_list[-50:]
        avg = sum(sample_epochs) / len(sample_epochs)
        self._average_duration_steps_list.append(avg)

    def save_avg_duration_steps_plot(self, save_path: str) -> None:
        Utils.save_plot(
            xdata=list(range(len(self._average_duration_steps_list))),
            ydata=self._average_duration_steps_list,
            title="Average Duration steps",
            xlabel="Epoch",
            ylabel="Average Duration steps",
            save_path=save_path,
            plot_type="line"
        )
